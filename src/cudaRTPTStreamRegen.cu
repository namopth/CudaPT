...
namespace cudaRTPTStreamRegen
{
	...
	__global__ void pt_traceSample_kernel(RTVertex* vertices, RTTriangle* triangles, RTMaterial* materials, CURTTexture* textures, PTPathVertex** pathStream, uint activePathStreamSize, PTPathVertex** pathRegenStream, uint activePathRegenStreamSize)
	{
		uint x = blockIdx.x * blockDim.x + threadIdx.x;
		if (x >= activePathStreamSize + activePathRegenStreamSize) return;
		PTPathVertex* procVertex = (x >= activePathStreamSize) ? pathRegenStream[x-activePathStreamSize] : pathStream[x];
		if (procVertex->isTerminated) return;
		...
		float3 camPos, float3 camDir, float3 camUp, float3 camRight, float fov, float width, float height)
	{
		...
		if (ind < pathQueueRegenReinitSize)
		{
			...
			float3 dir = normalize(camRight * au + camUp * av + camDir);
			PTPathVertex regenSample = PTPathVertex(false, assignSample->pathPixel, dir, camPos, RAYTYPE_EYE, assignSample->randState);
			regenSample.pathAccumSample = assignSample->pathAccumSample;
			regenSample.pathSampleN = assignSample->pathSampleN;
			*assignSample = regenSample;
		}
	}
		...
		, float width, float height, float* result)
	{
		...
		while (g_uPathQueueCur < g_uPathQueueSize || activePathStreamSize > 0)
		{
			...
			pt_traceSample_kernel << < dim3(ceil((float)(activePathStreamSize + activeRegenPathStreamSize) / (float)block1.x), 1, 1), block1 >> > (g_devVertices, g_devTriangles, g_devMaterials, g_devTextures, g_devPathStream, activePathStreamSize
				, g_devPathQueueRegen, activeRegenPathStreamSize);
			cudaDeviceSynchronize();

			//compact regenpathstream and find regenpathstreamsize value
			if (activePathStreamSize < maxActivePathStreamSize)
			{
				PTPathVertex** compactRegenStreamEndItr = thrust::remove_if(thrust::device, g_devPathQueueRegen, g_devPathQueueRegen + activeRegenPathStreamSize, is_terminated());
				uint terminatedRegenPathStreamSize = activeRegenPathStreamSize - (compactRegenStreamEndItr - g_devPathQueueRegen);
				if (terminatedRegenPathStreamSize > 0)
					thrust::copy(thrust::device, g_devPathQueueRegen + activeRegenPathStreamSize, g_devPathQueueRegen + g_uPathQueueRegenSize, compactRegenStreamEndItr);
