...
namespace cudaRTBDPTStreamAdapEmp
{
...
void updateLightTriCudaMem(RTScene* scene)
{
	...
	__global__ void pt_applyPathQueueResult_kernel(PTPathVertex* pathQueue, uint pathQueueSize, uint width, uint height, uint frameN, float* result, float* squareResult, float* varResult, uint* sampleResultN)
	{
		uint x = blockIdx.x * blockDim.x + threadIdx.x;
		...
		if (pathQueue[x].pathSampleN > 0)
		{
			...
			if (tempNextSampleResultN > sampleResultN[ind])
			{
				...
				float3 squareSampleResult = vecMul(sampleResult, sampleResult);
				float potentialResult = 1.f - pathQueue[x].pathAccumPotential;
				float resultInf = 1.f / (float)(tempNextSampleResultN);
				float oldInf = sampleResultN[ind] * resultInf;
				...
				result[ind * 3 + 1] = max(resultInf * sampleResult.y + oldInf * result[ind * 3 + 1], 0.f);
				result[ind * 3 + 2] = max(resultInf * sampleResult.z + oldInf * result[ind * 3 + 2], 0.f);
				squareResult[ind * 3] = max(resultInf * squareSampleResult.x + oldInf * squareResult[ind * 3], 0.f);
				squareResult[ind * 3 + 1] = max(resultInf * squareSampleResult.y + oldInf * squareResult[ind * 3 + 1], 0.f);
				squareResult[ind * 3 + 2] = max(resultInf * squareSampleResult.z + oldInf * squareResult[ind * 3 + 2], 0.f);
				sampleResultN[ind] = tempNextSampleResultN;
			}
		}
	}

	__global__ void pt_calculateEmpiricalSquareError_kernel(float* sampleData, float* squareSampleData, uint* sampleResultN, float* resultData, uint dataSize)
	{
		uint x = blockIdx.x * blockDim.x + threadIdx.x;
		if (x >= dataSize)
			return;
		uint sampleN = sampleResultN[x];
		resultData[x] = ((squareSampleData[x * 3] - sampleData[x * 3] * sampleData[x * 3])
			+ (squareSampleData[x * 3 + 1] - sampleData[x * 3 + 1] * sampleData[x * 3 + 1])
			+ (squareSampleData[x * 3 + 2] - sampleData[x * 3 + 2] * sampleData[x * 3 + 2])
			) * (float)sampleN / ((float)(sampleN - 1) * 3.f);
		resultData[x] = fminf(resultData[x], 1.0f);
	}
	...
	void CleanMem()
	{
		...
		CUFREE(g_devSquareResultData);
		CUFREE(g_devTempResult);
		...
		, float width, float height, float* result)
	{
		...
		if (!g_devFilterGaussianConst)
		{
			...
			cudaBilateralFilter::updateGaussian(g_devFilterGaussianConst, g_fFilterEuD, g_uiFilterRadius);
		}
		...
		if (g_uCurFrameN < 5)
		{
			...
			pt_applyPathQueueResult_kernel << < dim3(ceil((float)useQueueSize / (float)block1.x), 1, 1), block1 >> >
				(g_devPathQueue, useQueueSize, width, height, g_uCurFrameN, g_devResultData, g_devSquareResultData, g_devResultVarData, g_devSampleResultN);
			//HANDLE_ERROR(cudaEventRecord(stop, 0));
			//HANDLE_ERROR(cudaEventSynchronize(stop));
			//HANDLE_ERROR(cudaEventElapsedTime(&time, start, stop));
		...
		else
		{
			...
			//HANDLE_ERROR(cudaEventCreate(&start));
			//HANDLE_ERROR(cudaEventCreate(&stop));
			//cudaBilateralFilter::bilaterial_kernel << < renderGrid, block2 >> >(g_devResultData, width, height, g_fFilterEuD, g_uiFilterRadius, g_devFilterGaussianConst, g_devTempResult);

			// calculate sampling map from converged result
			pt_calculateEmpiricalSquareError_kernel << < dim3(ceil((float)(width * height) / (float)block1.x), 1, 1), block1 >> > (g_devResultData, g_devSquareResultData, g_devSampleResultN, g_devResultVarData, (uint)(width * height));
			//pt_calculateSquareError_kernel << < dim3(ceil((float)(width * height) / (float)block1.x), 1, 1), block1 >> > (g_devTempResult, g_devResultData, g_devResultVarData, (uint)(width * height));
			...
			for (auto pathQueueSize : pathQueuesSize)
			{
				if (*g_enumDebugMode.GetUint() == 1)
				{
					pt_debugTracedPathQueueResult_kernel << < dim3(ceil((float)pathQueueSize / (float)block1.x), 1, 1), block1 >> >
						(g_devPathQueue + accumPathQueueSize, pathQueueSize, width, height, g_uCurFrameN, g_devResultData, g_devSquareResultData, g_devResultVarData, g_devSampleResultN);
				}
				else
				{
		...
		{
			pt_applyPixelProbToResult_kernel << < renderGrid, block2 >> >(width, height, g_devTempResult, g_devResultVarData, (*g_enumDebugMode.GetUint() == 3) ? *g_fMinTraceProb.GetFloat() : 0.f);
		}

		// Copy result to host
		if (*g_enumDebugMode.GetUint() == 4 || *g_enumDebugMode.GetUint() == 2 || *g_enumDebugMode.GetUint() == 3)
		{
			cudaMemcpy(result, g_devTempResult, g_resultDataSize, cudaMemcpyDeviceToHost);
		}
		else
		{
